...
public final class SaguaroMain extends JavaPlugin {
	...
	public void onEnable() {
        ...
        tpsMeterTask = new TpsMeter(this).runTaskTimer(this, tpsSamplePeriod*20, tpsSamplePeriod*20);
        ...
        if (getConfig().getBoolean("file-output-enabled",false)) {
        	fileWriterTask = new FileWriter(this).runTaskTimer(this, 0, fileUpdatePeriod*20);
        }
        ...
        if (getConfig().getBoolean("telnet-enabled",true)) {
        	...
        	try {
        		...
        		getLogger().info("Telnet server listening on port " + telnetPort);
        	} catch (IOException e) {
