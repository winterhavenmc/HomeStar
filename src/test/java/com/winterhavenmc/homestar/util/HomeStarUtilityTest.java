...
class HomeStarUtilityTest {
	...
	void createTest() {
		...
		// create test item
		ItemStack testItem = plugin.homeStarUtility.create();
	...
	void isItemTest() {
		...
		assertTrue(plugin.homeStarUtility.isItem(testItem), "testItem is not a proper HomeStar item.");

		// assert null ItemStack is does not pass isItem test
		assertFalse(plugin.homeStarUtility.isItem(null), "null ItemStack passed isItem test.");

		// create item without metadata
		ItemStack itemStack = new ItemStack(Material.DIRT);
		assertFalse(plugin.homeStarUtility.isItem(itemStack), "item stack without metadata passed isItem test.");
	}
	...
	void getDefaultItemStackTest() {
		...
		ItemStack testItem = plugin.homeStarUtility.getDefaultItemStack();
	...
	void setMetaDataTest() {
		...
		plugin.homeStarUtility.setMetaData(testItem);
		...
		assertEquals("HomeStar", ChatColor.stripColor(testItem.getItemMeta().getDisplayName()), "new item display name is not HomeStar.");
		assertNotNull(itemMeta.getLore(), "item lore is null.");
		assertFalse(itemMeta.getLore().isEmpty(), "item lore is empty.");
		assertEquals("Use to Return to Home", ChatColor.stripColor(String.join(" ", itemMeta.getLore())), "new item stack lore does not match default lore.");
	}
